    ...
    <div class="container">
        <div class="archive-header">
            <div class="row align-items-center">
                <div class="col-xl-6">
                    @php
                        $brandId = request()->segment(2); // Get the brand ID from the URL
                        $brand = App\Models\Brand::find($brandId); // Find the category by ID
                    @endphp
                    @if($brand)
                        <h1 class="mb-15"> {{ $brand->name }} </h1>
                        <div class="breadcrumb">
                            <a href="{{ '/' }}" rel="nofollow"><i class="fi-rs-home mr-5"></i>Home</a>
                            <span></span> Shop <span></span> {{ $brand->name }}
                        </div>
                    @endif
                </div>
            </div>
    ...
    <div class="row flex-row-reverse">
        ...
        <div class="col-lg-1-5 primary-sidebar sticky-sidebar">
            <div class="sidebar-widget widget-category-2 mb-30">
                ...
                @php
                  $brands = App\Models\Brand::has('products')->withCount('products')->get();
                @endphp
                <ul>
                  @foreach ($brands as $brand)
                    <li>
                      <a href="{{ route('brand.products', ['id' => $brand->id, 'slug' => $brand->slug]) }}">
                        <img src="{{ asset('upload/brands/'.$brand->image) }}" alt="{{ $brand->name }}" />
